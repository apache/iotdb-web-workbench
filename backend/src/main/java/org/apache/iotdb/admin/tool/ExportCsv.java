...
public class ExportCsv {
    ...
    public String exportCsv(String host, Integer port, String username, String password, String sql, String timeZone) throws BaseException {
    ...
    private String dumpResult(String sql, Session session, ZoneId zoneId) throws BaseException {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(tf))) {
            ...
            log.info("从sql语句：" + sql + "成功导出" + line + "行数据，用时" + (System.currentTimeMillis() - startTime) + "ms");
            return fileName;
        } catch (IoTDBConnectionException e) {
            throw new BaseException(ErrorCode.GET_SESSION_FAIL, ErrorCode.GET_SESSION_FAIL_MSG);
        } catch (IOException e) {
            ...
            throw new BaseException(ErrorCode.EXPORT_CSV_FAIL, ErrorCode.EXPORT_CSV_FAIL_MSG + e.getMessage());
        }
    }

    private void writeMetadata(BufferedWriter bw, List<String> columnNames)
            throws IOException {
    ...
    private int writeResultSet(SessionDataSet rs, BufferedWriter bw, ZoneId zoneId, boolean isDataType)

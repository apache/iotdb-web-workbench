...
class QueryControllerTest {
    ...
    void query() throws Exception {
        ...
        RequestBuilder request = MockMvcRequestBuilders
            .post("http://127.0.0.1:8080/api/servers/1/querySql")
            .contextPath("/api")
            .content("{\n" +
                "  \"sqls\": [\n" +
                "    \"show storage group\",\n" +
    ...
    void saveQuery() throws Exception {
        mvc = MockMvcBuilders.webAppContextSetup(wac).build();
        RequestBuilder request = MockMvcRequestBuilders
            .post("http://127.0.0.1:8080/api/servers/1/query")
            .contextPath("/api")
            .content("{\n" +
                "  \"connectionId\": 1,\n" +
                "  \"queryName\": \"testQ\",\n" +
    ...
    void getQueries() throws Exception {
        mvc = MockMvcBuilders.webAppContextSetup(wac).build();
        RequestBuilder request = MockMvcRequestBuilders
            .get("http://127.0.0.1:8080/api/servers/1/query")
            .contextPath("/api")
            .header("Authorization", token);
        mvc.perform(request).andExpect(MockMvcResultMatchers.status().isOk());
    }
    ...
    void deleteQuery() throws Exception {
        mvc = MockMvcBuilders.webAppContextSetup(wac).build();
        RequestBuilder request = MockMvcRequestBuilders
            .delete("http://127.0.0.1:8080/api/servers/1/query/4")
            .contextPath("/api")
            .header("Authorization", token);
        mvc.perform(request).andExpect(MockMvcResultMatchers.status().isOk());
    }
